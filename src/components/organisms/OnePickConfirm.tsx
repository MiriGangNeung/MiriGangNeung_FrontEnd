    ...
    <div className="flex min-h-[calc(100dvh-var(--app-header))] flex-col px-4 pb-40 pt-7 sm:px-6 sm:pb-8 sm:pt-8 lg:pb-2.5 lg:pt-2.5">
      <div className="mx-auto flex w-full max-w-[1180px] flex-1 flex-col">
        <div>
          <button
            onClick={onBack}
            className="-ml-1 mb-1.5 inline-flex items-center gap-1 rounded-full px-1 py-0.5 text-[13px] font-semibold text-ink-soft hover:text-brand"
          >
            <ChevronLeft size={16} strokeWidth={2} /> 장소 다시 고르기
          </button>
          <h1 className="m-0 text-pretty text-[22px] font-extrabold leading-[1.3] -tracking-[.7px] sm:text-[23px] sm:-tracking-[.7px]">
            합성에 사용할 단 하나의 <br className="sm:hidden" />
            원픽 장소를 선택해주세요.
          </h1>
          <p className="mt-1.5 text-pretty text-[13px] leading-[1.5] text-ink-muted">
        ...
        <div className="sticky bottom-3 z-30 mt-8 flex flex-wrap items-center gap-x-5 gap-y-3 rounded-[20px] border border-line bg-white px-4 py-3.5 shadow-bar sm:static sm:mt-3 sm:px-6 sm:py-3">
          ...
          {activeIsSelected ? (
            <button
              onClick={onNext}
              className="flex h-[42px] w-full shrink-0 items-center justify-center gap-2 whitespace-nowrap rounded-full bg-brand px-7 text-[15px] font-bold text-white shadow-cta hover:bg-brand-dark sm:w-auto"
            >
            ...
            <button
              ...
              className="flex h-[42px] w-full shrink-0 items-center justify-center gap-2 whitespace-nowrap rounded-full bg-brand px-7 text-[15px] font-bold text-white shadow-cta hover:bg-brand-dark disabled:cursor-not-allowed disabled:bg-fill disabled:text-ink-soft disabled:shadow-none sm:w-auto"
            >
              {activePlaceLabel}
