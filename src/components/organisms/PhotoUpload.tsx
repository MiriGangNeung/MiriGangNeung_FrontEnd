...
export function PhotoUpload({
  ...
  return (
    <div className="min-h-[calc(100dvh-var(--app-header))] px-4 pb-16 pt-7 sm:px-6 sm:pt-10">
      <div className="mx-auto max-w-[1180px]">
        <h1 className="m-0 text-xl font-extrabold -tracking-[.5px] sm:text-2xl sm:-tracking-[.6px]">
          내 사진을 올리고 합성을 시작하세요
        </h1>
        ...
        <div className="mt-5 grid grid-cols-1 items-start gap-7 sm:mt-[22px] lg:grid-cols-[1fr_380px] lg:items-stretch">
          <div className="rounded-[20px] border border-line bg-white p-4 sm:p-[22px]">
            <input
              ref={fileInputRef}
              type="file"
              accept="image/*"
            ...
            <div className="mt-[18px] flex flex-wrap items-center gap-2 border-t border-line pt-[18px]">
              <button
                onClick={openFilePicker}
                className="flex h-10 shrink-0 items-center gap-1.5 whitespace-nowrap rounded-full border border-line bg-white px-4 text-[13px] font-semibold text-ink-muted hover:border-brand hover:text-brand"
              >
              ...
              <button
                ...
                disabled={!photoFile}
                className="flex h-10 shrink-0 items-center gap-1.5 whitespace-nowrap rounded-full border border-line bg-white px-4 text-[13px] font-semibold text-ink-muted hover:border-coral hover:text-coral disabled:cursor-not-allowed disabled:opacity-50"
              >
                <X size={16} strokeWidth={1.8} /> 삭제
              </button>
              <div className="hidden flex-1 sm:block" />
              <span className="basis-full text-xs text-ink-soft sm:basis-auto">
                JPG PNG 10MB이하 사진을 권장합니다.
              </span>
              <span className="hidden basis-full text-xs text-ink-soft sm:basis-auto">
                JPG · PNG · 10MB 이하 · 얼굴이 정면으로 보이는 사진을 권장합니다
              </span>
            </div>
          </div>

          <aside className="flex flex-col rounded-[20px] border border-line bg-white p-5 shadow-panel sm:p-6 lg:sticky lg:top-[calc(var(--app-header)+24px)]">
            {phase === 'ready' && (
              <>
                ...
                <button
                  ...
                  className="mt-5 flex h-[52px] w-full items-center justify-center gap-2 rounded-full bg-brand text-[15px] font-bold text-white shadow-cta hover:bg-brand-dark disabled:bg-fill disabled:text-ink-soft disabled:shadow-none lg:mt-auto"
                >
            ...
            {phase === 'done' && (
              <>
                ...
                <h2 className="mt-4 text-lg font-extrabold -tracking-[.4px]">
                  합성이 완료되었어요
                </h2>
                <p className="mt-2 text-[13px] leading-[1.7] text-ink-muted">
  ...
  return (
    <div className="flex flex-col">
      <div className="mb-3 flex h-6 items-center gap-2">
        <span
          className={`shrink-0 whitespace-nowrap rounded-full px-2.5 py-1 text-xs font-bold ${badgeTone === 'coral' ? 'bg-coral-tint text-coral' : 'bg-brand-tint text-brand'}`}
        >
          {badge}
        </span>
      </div>
      ...
      >
        <ImageSlot src={src} placeholder={uploadPlaceholder ?? placeholder} />
      </div>
    </div>
  );
}
